...
using AcceptHandler = std::function<void(
  std::shared_ptr<firelink::Socket> caller, std::shared_ptr<firelink::Socket> accepted_socket,
  const Endpoint& local_endpoint, const Endpoint& peer_endpoint, std::shared_ptr<void> user_op_data,
  ErrorCode error, AcceptTag tag)>;

using ConnectHandler =
  std::function<void(std::shared_ptr<firelink::Socket> caller, std::shared_ptr<void> user_op_data,
                     ErrorCode error, ConnectTag tag)>;

using ReadHandler = std::function<void(
  std::shared_ptr<firelink::Socket> caller, std::span<std::byte> user_buffer,
  std::shared_ptr<void> user_op_data, ErrorCode error, std::int32_t bytes_transferred, ReadTag tag)>;

using WriteHandler = std::function<void(
  std::shared_ptr<firelink::Socket> caller, std::span<std::byte> user_buffer,
  std::shared_ptr<void> user_op_data, ErrorCode error, std::int32_t bytes_transferred, WriteTag tag)>;
...
class FIRELINK_CLASS_API Socket
{
  ...
  virtual ErrorCode get_socket_option(SocketOptionLevel level, SocketOption option,
                                      std::span<std::byte> value, std::size_t& value_size_out) = 0;
  ...
  inline NativeHandle get_native_handle() const
  {
    return socket_;
  }
  inline AddressFamily get_addr_family() const
  {
    return addr_family_;
  }
  inline SocketType get_sock_type() const
  {
    return sock_type_;
  }
  inline Protocol get_protocol() const
  {
    return protocol_;
  }
  ...
  // Asynchronous API
  virtual ErrorCode start_accept(std::shared_ptr<firelink::Socket> accept_socket,
                                 std::shared_ptr<void> user_op_data = nullptr,
                                 AcceptHandler handler = AcceptHandler{}) = 0;

  virtual ErrorCode start_connect(const Endpoint& dst, std::shared_ptr<void> user_op_data = nullptr,
                                  ConnectHandler handler = ConnectHandler{}) = 0;

  virtual ErrorCode start_recv(std::span<std::byte> buffer,
                               std::shared_ptr<void> user_op_data = nullptr,
                               ReadHandler handler = ReadHandler{}) = 0;

  virtual ErrorCode start_recv_from(std::span<std::byte> buffer,
                                    std::shared_ptr<void> user_op_data = nullptr,
                                    ReadHandler handler = ReadHandler{}) = 0;

  virtual ErrorCode start_send(std::span<std::byte> data, std::shared_ptr<void> user_op_data = nullptr,
                               WriteHandler handler = WriteHandler{}) = 0;

  virtual ErrorCode start_send_to(std::span<std::byte> data, const Endpoint& dst,
                                  std::shared_ptr<void> user_op_data = nullptr,
                                  WriteHandler handler = WriteHandler{}) = 0;

  virtual ErrorCode start_disconnect(bool reuse_socket, std::shared_ptr<void> user_op_data = nullptr,
                                     DisconnectHandler handler = DisconnectHandler{}) = 0;
