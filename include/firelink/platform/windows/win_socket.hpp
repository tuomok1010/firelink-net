...
struct IOData
{
  ...
  std::variant<AcceptHandler, ConnectHandler, ReadHandler, WriteHandler, DisconnectHandler>
    user_handler_;
...
class WinSocket : public Socket, public std::enable_shared_from_this<WinSocket>
{
  ...
  ErrorCode get_socket_option(SocketOptionLevel level, SocketOption option,
                              std::span<std::byte> value, std::size_t& value_size_out) override;
  ...
  static ErrorCode addr_and_port_to_str(LPSOCKADDR_STORAGE addr, std::string& addr_str,
                                        std::string& port_str);
  static ErrorCode str_to_addr4(std::string_view addr_str, std::string_view port_str,
                                LPSOCKADDR_IN addr);
  static ErrorCode str_to_addr6(firelink::AddressFamily src_addr_family,
                                std::string_view src_addr_str, std::string_view src_port_str,
                                LPSOCKADDR_IN6 dst_addr);

  public:
  // Asynchronous API
  ErrorCode start_accept(std::shared_ptr<firelink::Socket> accept_socket,
                         std::shared_ptr<void> user_op_data = nullptr,
                         AcceptHandler handler = AcceptHandler{}) override;

  ErrorCode start_connect(const Endpoint& dst, std::shared_ptr<void> user_op_data = nullptr,
                          ConnectHandler handler = ConnectHandler{}) override;
  ...
  ErrorCode start_send(std::span<std::byte> data, std::shared_ptr<void> user_op_data = nullptr,
                       WriteHandler handler = WriteHandler{}) override;

  ErrorCode start_send_to(std::span<std::byte> data, const Endpoint& dst,
                          std::shared_ptr<void> user_op_data = nullptr,
                          WriteHandler handler = WriteHandler{}) override;

  ErrorCode start_disconnect(bool reuse_socket, std::shared_ptr<void> user_op_data = nullptr,
                             DisconnectHandler handler = DisconnectHandler{}) override;
  ...
  private:
  static ErrorCode get_acceptex_sockaddrs(PVOID buffer, LPSOCKADDR_STORAGE local_addr,
                                          LPSOCKADDR_STORAGE remote_addr, DWORD local_addr_len,
